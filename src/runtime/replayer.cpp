...
void flush_group(
    ...
    const auto record = store.commit(branch, tx, verifier);
    if (!record.has_value() || !record->accepted) {
        result.errors.push_back(ReplayError{
            group->first_line,
            "epoch",
            fmt::format("runtime replay commit for epoch {} was rejected", group->epoch.value)
        });
    } else {
...
RuntimeReplayResult RuntimeReplayer::replay_into(
    WorldStore& store,
    BranchId branch,
    ...
    result.branch_name = store.branch(branch).name;
    ...
    while (std::getline(input, line)) {
        ...
        maybe_reset_world(store.mutable_world(branch), fields);

        if (is_state_event(event) || is_empty_transition_marker(event)) {
            if (group.has_value() && group->epoch != epoch) {
                flush_group(group, result, store, branch, verifier);
            }
            if (!group.has_value()) {
    ...
    flush_group(group, result, store, branch, verifier);
    result.final_hash = store.world(branch).canonical_hash();
    return result;
}
