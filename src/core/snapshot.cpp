...
void set_attribute(std::vector<Attribute>& attributes, Attribute attribute) {
    const auto iter = std::ranges::find_if(attributes, [&](const Attribute& existing) {
        return existing.key == attribute.key;
    });
    if (iter == attributes.end()) {
        attributes.push_back(std::move(attribute));
    } else {
        *iter = std::move(attribute);
    }
    sort_attributes(attributes);
}

void remove_attribute(std::vector<Attribute>& attributes, const std::string& key) {
    attributes.erase(
        std::remove_if(attributes.begin(), attributes.end(), [&](const Attribute& attribute) {
...
std::expected<void, DeltaMergeError> reject_internal_type_conflicts(const Delta& delta) {
    ...
    for (const auto& update : delta.updates_view()) {
...
std::map<ObjectId, TempObjectId, bool (*)(ObjectId, ObjectId)> synthetic_temp_map(
    ...
    for (const auto& create : delta.creates_view()) {
...
TempObjectId next_temp_id(const Delta& first, const Delta& second) noexcept {
    std::uint32_t next = 1;
    for (const auto& create : first.creates_view()) {
        next = std::max(next, create.temp_id.value + 1);
    }
    for (const auto& create : second.creates_view()) {
        next = std::max(next, create.temp_id.value + 1);
    }
...
std::expected<WorldSnapshot, OverlayError> SnapshotOverlay::apply(const Delta& delta) const {
    ...
    for (const auto& op : delta.ops) {
        ...
        case OperationKind::CreateObject: {
            ...
            object.attributes = create.attributes;
            sort_attributes(object.attributes);
        ...
        case OperationKind::SetObjectAttribute: {
            ...
            set_attribute(object->attributes, update.attribute);
            break;
        }
        case OperationKind::RemoveObjectAttribute: {
            ...
            remove_attribute(object->attributes, update.key);
            break;
        }
        case OperationKind::CreatePointer: {
            ...
            pointer.id = PointerId{next_pointer++};
            pointer.from = *from;
            pointer.to = *to;
            pointer.relation = link.relation;
            pointer.causal_role = link.causal_role;
            pointer.weight = link.weight;
            pointer.born_at = out.epoch;
            pointer.law_domain = link.law_domain.empty() ? "core" : link.law_domain;
            pointer.attributes = link.attributes;
            sort_attributes(pointer.attributes);
            out.pointers.push_back(std::move(pointer));
            break;
        }
        ...
        case OperationKind::SetPointerWeight: {
            ...
            auto* pointer = mutable_pointer(out, update.id);
            if (pointer == nullptr) {
                return std::unexpected(OverlayError::InvalidPointerRemove);
            }
            pointer->weight = update.weight;
            break;
        }
        case OperationKind::SetPointerAttribute: {
            ...
            set_attribute(pointer->attributes, update.attribute);
            break;
        }
        case OperationKind::RemovePointerAttribute: {
            ...
            remove_attribute(pointer->attributes, update.key);
            break;
        }
...
merge_sequential(const WorldSnapshot& base, const Delta& first, const Delta& second) {
    ...
    Delta out;
    for (auto op : first.ops) {
        out.ops.push_back(std::move(op));
    }
    for (auto op : normalized_second.ops) {
        out.ops.push_back(std::move(op));
    }
